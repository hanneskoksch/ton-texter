...
As already shown in the code snippet above Whisper provides several different model sizes. In the beginning we had chosen to go with the *base* model, thanks to [the implementation of performance improvements](https://ton-texter.de/blog) we later chose to go with the *small* model to further improve our transcription.
...
If the console output is *"Using CPU instead"* we have to investigate the setup of our device, drivers and torch installation. 
...
With the *ProgressHook* we can monitor the progress of the pipeline.
...
For the generation of our word file we use the *python-docx* library.
